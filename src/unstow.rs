...
pub(crate) fn unstow_path<'a>(
    ...
    if !target_exist || !target_is_symlink || !is_valid_symlink {
        let cause = {
            if !target_exist {
                "Target not found"
            } else if !target_is_symlink {
                "Target not a symlink"
            } else if !is_valid_symlink {
    ...
    if backup_exist {
        operations.push_back(FSOperation::Restore {
...
mod test_unstow {
    ...
    fn test_valid_link_file() {
        with_test_directories(
            "unstow_test_valid_link_file".as_ref(),
    ...
    fn test_valid_link_file_with_backup() {
        with_test_directories(
            "unstow_test_valid_link_file_with_backup".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = target.join("file.txt");
                ...
                assert_eq!(
                    iter.next().unwrap(),
                    &FSOperation::Restore {
                        backup: backup_target_file.to_path_buf(),
    ...
    fn test_unvalid_link_file() {
        with_test_directories(
            "unstow_test_unvalid_link_file".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = target.join("file.txt");
                ...
                assert_eq!(
                    ...
                    &FSOperation::Nothing {
                        ...
                        cause: "Target symlink invalid".to_owned()
                    }
                );
                assert_eq!(iter.next(), None);
            },
        ...
        with_test_directories(
            ...
            |source: &PathBuf, target: &PathBuf| {
                ...
                assert_eq!(
                    ...
                    &FSOperation::Nothing {
                        path: target_file.to_path_buf(),
                        cause: "Target not a symlink".to_owned()
                    }
                );
                assert_eq!(iter.next(), None);
            },
    ...
    fn test_file_notfound() {
        with_test_directories(
            "unstow_test_file_notfound".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = target.join("file.txt");
                ...
                assert_eq!(
                    ...
                    &FSOperation::Nothing {
                        path: target_file.to_path_buf(),
                        cause: "Target not found".to_owned()
                    }
                );
                assert_eq!(iter.next(), None);
            },
    ...
    fn test_valid_link_directory() {
        with_test_directories(
            "unstow_test_valid_link_directory".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
    ...
    fn test_valid_link_directory_with_backup() {
        with_test_directories(
            "unstow_test_valid_link_directory_with_backup".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_directory_to("subdir", source.as_path()).unwrap();
                let target_file = target.join("subdir");
                let backup_target_file =
                    add_directory_to("subdir.backup", target.as_path()).unwrap();
    ...
    fn test_unvalid_link_directory() {
        with_test_directories(
            "unstow_test_unvalid_link_directory".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_directory_to("subdir", source.as_path()).unwrap();
                let target_file = target.join("subdir");
