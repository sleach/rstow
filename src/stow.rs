...
pub(crate) fn stow_path<'a>(
    ...
    force: bool,
    backup: bool,
    operations: &'a mut Vector<FSOperation>,
    ...
    let symlink_operation = FSOperation::CreateSymlink {
    ...
    match (target_exist, target_is_symlink, target_is_directory, force) {
        (true, true, _, _) => {
            ...
            if is_valid_symlink {
                ...
                debug!(
                    "Valid symlink {} already exist, nothing to do",
                    target_path.display()
                );
                operations.push_back(FSOperation::Nothing {
                    path: target_path.to_path_buf(),
                    cause: "Valid symbolic link".to_owned(),
                });
                stop_if_directory()
            } else {
                if target_is_directory {
                    if force {
                        ...
                        operations
                            .push_back(FSOperation::BreakDirectoryLink(target_path.to_path_buf()));
                        Ok(TraversOperation::Continue)
                    } else {
                        debug!(
                            "Error: Invalid symlink {} already exist on directory.",
                            target_path.display()
                        );
                    ...
                    if force {
                        ...
                        log!(
                            Level::Warn,
                            "Path symlink {} already exist and will be override",
                            target_path.display()
                        );
                        ...
                        debug!(
                            "Error: Invalid symlink {} already exist on file without force flag.",
                            target_path.display()
                        );
                        Err(AppError::StowPathError {
                            source: ErrorPath::from(source_path),
        ...
        (true, false, false, true) => {
            ...
            if backup {
                ...
                log!(
                    Level::Warn,
                    "Path {} already exist and will be override !",
                    target_path.display()
                );
                operations.push_back(FSOperation::Delete(target_path.to_path_buf()));
            }
        ...
        (true, false, false, false) => {
            debug!(
                "Error: Invalid target file {} already exist without force flag.",
                target_path.display()
            );
            ...
            Err(AppError::StowPathError {
                source: ErrorPath::from(source_path),
                target: ErrorPath::from(target_path),
                cause: "Target file already physically exist. Set -f flag to force override"
                    .to_string(),
            })
        }
        (true, false, true, _) => {
            debug!(
                "Target directory {} exist. Continue on children",
                target_path.display()
            );
        ...
        (false, _, true, _) => {
            ...
            if config.symlink_current_dir {
                debug!(
                    "Target directory {} not exist. Create symlink.",
                    target_path.display()
                );
                operations.push_back(symlink_operation);
                Ok(TraversOperation::StopPathRun)
                ...
                debug!(
                    "Target directory {} not exist. Create directory forced by configuration.",
                    target_path.display()
                );
        ...
        (false, _, _, _) => {
            debug!(
                "Target file {} not exist. Create symlink.",
                target_path.display()
            );
            operations.push_back(symlink_operation);
            stop_if_directory()
...
mod test_stow {
    ...
    fn test_file() {
        with_test_directories(
            ...
            |source: &PathBuf, target: &PathBuf| {
                ...
                let result = stow_path(
                    source_file.as_path(),
                    target_file.as_path(),
                    NO_FORCE,
                    NO_BACKUP,
                    operations.borrow_mut(),
                );
    ...
    fn test_directory_with_config() {
        with_test_directories(
            "test_directory_with_config".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_dir = add_directory_to("subDir", source.as_path()).unwrap();
                let target_dir = target.join("subDir");

                let mut config_file =
                    File::create(source_dir.as_path().join(RSTOW_FILE_NAME)).unwrap();
                let content = r#"
    ...
    fn test_existing_directory() {
        with_test_directories(
            "test_existing_directory".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
    ...
    fn test_existing_file_with_force() {
        with_test_directories(
            "test_existing_file_with_force".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = add_file_to("file.txt", target.as_path()).unwrap();
                ...
                assert_eq!(
                    iter.next().unwrap(),
                    &FSOperation::Delete(target_file.to_path_buf())
                );
                assert_eq!(
                    iter.next().unwrap(),
                    &FSOperation::CreateSymlink {
    ...
    fn test_existing_directory_with_force() {
        with_test_directories(
            "test_existing_directory_with_force".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_directory_to("subDir", source.as_path()).unwrap();
                let target_file = add_directory_to("subDir", target.as_path()).unwrap();
    ...
    fn test_existing_file_with_force_backup() {
        with_test_directories(
            "test_existing_file_with_force_backup".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = add_file_to("file.txt", target.as_path()).unwrap();
                ...
                assert_eq!(
                    iter.next().unwrap(),
                    &FSOperation::Backup(target_file.to_path_buf())
                );
                assert_eq!(
                    iter.next().unwrap(),
                    &FSOperation::CreateSymlink {
    ...
    fn test_existing_directory_with_force_backup() {
        with_test_directories(
            "test_existing_directory_with_force_backup".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_directory_to("subDir", source.as_path()).unwrap();
                let target_file = add_directory_to("subDir", target.as_path()).unwrap();
    ...
    fn test_existing_valid_link_file() {
        with_test_directories(
            "test_existing_valid_link_file".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = target.join("file.txt");
    ...
    fn test_existing_valid_link_directory() {
        with_test_directories(
            "test_existing_valid_link_directory".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_directory_to("subDir", source.as_path()).unwrap();
    ...
    fn test_existing_invalid_link_file() {
        with_test_directories(
            "test_existing_invalid_link_file".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = target.join("file.txt");

                let other_source_dir: PathBuf =
                    source.parent().unwrap().to_path_buf().join("somewhere");
    ...
    fn test_existing_invalid_link_directory() {
        with_test_directories(
            "test_existing_invalid_link_directory".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_directory_to("subDir", source.as_path()).unwrap();
                let target_file = target.join("subDir");
    ...
    fn test_existing_invalid_link_file_with_force() {
        with_test_directories(
            "test_existing_invalid_link_file_with_force".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_file_to("file.txt", source.as_path()).unwrap();
                let target_file = target.join("file.txt");
    ...
    fn test_existing_invalid_link_directory_with_force() {
        with_test_directories(
            "test_existing_invalid_link_directory_with_force".as_ref(),
            |source: &PathBuf, target: &PathBuf| {
                let source_file = add_directory_to("subDir", source.as_path()).unwrap();
                let target_file = target.join("subDir");
                ...
                assert_eq!(
                    iter.next().unwrap(),
                    &FSOperation::BreakDirectoryLink(target_file.to_path_buf())
                );
                assert_eq!(iter.next(), None);
            },
        );
